...
function addGoodsNode(container, url) {
	...
	imgadd.className = "badd";
	imgdel.className = "bdelete";
	imgtg.setAttribute('price', price.toString());
	imgadd.setAttribute('did', url[2]);
	imgdel.setAttribute('did', url[2]);

	imgtg.addEventListener("click", () => sendBot(imgtg));
	imgdel.addEventListener("click", () => motloh(imgdel));

	div.append(imgtg, imgadd, imgdel);
	container.appendChild(div);
}
