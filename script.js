...
if (!dchecker) {
    ...
    div_close.id = "close";
    div_checker.setAttribute("style", "z-index: 1000;text-align: center;width: 300px;height: 50px;position: fixed;padding: 5px;background-color: rgb(237 229 245);border: 4px solid rgb(121, 34, 204);border-radius: 0px 50px;");
    div_close.setAttribute("style", "cursor: pointer;text-align: center;border: 4px solid rgb(121,34,204);left: 270px;width: 30px;height: 30px;position: fixed;top: 0px;background-color: rgb(237 229 245);border-radius: 20px;");
    ...
    btn_checker.setAttribute("style", "text-align:center;padding: 5px;background-color:#ffffff;border:1px solid rgb(121, 34, 204);border-radius: 10px;");
    div_close.addEventListener('click', function () {
        document.getElementById('div_checker').remove();
    });
...
function getPrice() {
    ...
    linkarr.forEach(function (data) {
        ...
        if (foundprice === undefined) {
            foundprice = jobj['priceComponent']['origPrice']['multiCurrencyFormatPrice'];
        }
        data[3] = foundprice;
    ...
    let div_close = document.createElement('div');
    div_close.id = "close";
    div_close.setAttribute("style", "cursor: pointer;text-align: center;border: 4px solid rgb(121,34,204);left: 420px;width: 30px;height: 30px;position: fixed;top: 0px;background-color: rgb(237 229 245);border-radius: 20px;");
    div_close.innerHTML = "<B>x</B>";
    div_close.addEventListener('click', function () {
        document.getElementById('div_checker').remove();
    });
